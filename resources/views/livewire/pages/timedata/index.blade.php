...
with(fn () => ['timedataList' => TimeDataDB::wire_list($this->sortField,$this->sortDirection,$this->filter)->paginate(10)]);

//создано новое событие
on(['timedata-created'=>function ($name, $day) {
            ...
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
                <div class="flex">
                    <div class="hidden md:block p-3 md:w-[200px] lg:w-[300px] text-gray-500">
                        @if (is_array($timedataInfo))
                            <div>{{ __('Timedata Info') }}</div>
                        @else
                            <div class="">
                    ...
                    <div class="grow relative overflow-x-auto p-1">
                        <div class="flex">
                            @can('timedata.create')
                            <livewire:pages.timedata.create />
                            @endcan
                            <div class="p-2 flex">
                                <div class="p-1 flex-none">{{ __('Timedata filter') }}</div>
                        ...
                        @empty
                            <x-table.row>
                                <x-table.cell class="block text-center" colspan="6">
                                    {{ __('No timedata found') }}
                                </x-table.cell>
                            </x-table.row>
                        @endforelse
                    </x-table>
                    <div class="m-2"> {{ $timedataList->links() }} </div>
                    </div>
                    </div>
                </div>
            </div>
