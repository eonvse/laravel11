<div  {{ $attributes->merge(['class'=>'overflow-y-auto z-50 cursor-pointer dark:text-gray-300']) }}
    ...
    {{ $slot }}
    @isset($tooltip)
    <div x-cloak x-show="tooltip" {{ $attributes->merge(['class'=>'px-2 border fixed bg-yellow-100 text-black p-1 overflow-hidden transform transition-all rounded my-1 dark:bg-gray-500']) }}
